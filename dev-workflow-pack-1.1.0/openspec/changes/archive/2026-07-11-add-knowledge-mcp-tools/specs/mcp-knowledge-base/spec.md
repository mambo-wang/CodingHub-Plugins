...
MCP 客户端 SHALL 能够通过 `h3_coding_hub_kb_upload_document` 工具获取文档上传的 REST API 信息。此操作需要认证。

#### Scenario: 获取上传信息
- **WHEN** 客户端传入 `kbId`、`username`、`password`
- **THEN** 系统返回 REST API 上传接口信息（URL、HTTP 方法、Content-Type、表单字段、限制），客户端通过 HTTP multipart POST 直传文件
