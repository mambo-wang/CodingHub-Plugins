...
系统 SHALL 通过 REST API 提供论坛帖子的创建、查询、更新、删除能力。帖子的更新和删除权限为：创建者本人或管理员（ADMIN/SUPER_ADMIN）。非创建者且非管理员的用户 SHALL 收到 403 Forbidden。

帖子列表和详情接口 MUST 返回关联标签信息（`tags` 数组）。更新帖子时 MUST 支持修改标签关联。

#### Scenario 1: Guest browses post list
- **GIVEN** 数据库中有多个帖子，包含不同分类和标签
- **WHEN** 游客访问 `GET /api/forum/posts?page=0&size=10`
- **THEN** 返回帖子列表（分页），每条包含 id、title、authorName、categoryName、**tags**、createdAt，不包含 content

#### Scenario 2: Guest views post detail
- **GIVEN** 数据库中存在 id=1 的帖子，内容为 Markdown 格式，关联标签 [Python, AI]
- **WHEN** 游客访问 `GET /api/forum/posts/1`
- **THEN** 返回完整帖子信息，content 为原始 Markdown，**tags 数组包含关联的标签信息（id, name）**

#### Scenario 3: User creates post
- **GIVEN** 用户已登录，token 有效
- **WHEN** 用户提交 `POST /api/forum/posts`，包含 title、content、categoryId、tagIds
- **THEN** 创建帖子，返回 201，帖子的 authorId 为当前用户，标签关联写入成功

#### Scenario 4: Author or admin updates post
- **GIVEN** 用户 A 拥有 id=1 的帖子
- **WHEN** 用户 A 提交 `PUT /api/forum/posts/1`，更新 title、content **和 tagIds**
- **THEN** 帖子更新成功，返回 200，**标签关联替换为新的 tagIds 列表**
- **WHEN** 角色为 ADMIN 的用户 B 提交 `PUT /api/forum/posts/1`
- **THEN** 帖子更新成功，返回 200（管理员可编辑他人帖子）
- **WHEN** 角色为 USER 的用户 B 提交 `PUT /api/forum/posts/1`
- **THEN** 返回 403 Forbidden

#### Scenario 5: Author or admin deletes post
- **GIVEN** 用户 A 拥有 id=1 的帖子
- **WHEN** 用户 A 提交 `DELETE /api/forum/posts/1`
- **THEN** 帖子状态标记为 DELETED，返回 204
- **WHEN** 角色为 ADMIN 的用户 B 提交 `DELETE /api/forum/posts/1`
- **THEN** 帖子状态标记为 DELETED，返回 204（管理员可删除他人帖子）
- **WHEN** 角色为 USER 的用户 B 提交 `PUT /api/forum/posts/1`
- **THEN** 返回 403 Forbidden
...
#### Scenario 11: 更新帖子标签（新增）
- **GIVEN** 帖子 postId=1 当前关联标签 [2, 5]
- **WHEN** 帖子作者提交 `PUT /api/forum/posts/1`，传入 `tagIds: [2, 8]`
- **THEN** 帖子标签关联更新为 [2, 8]，旧标签 5 的 usage_count 减 1，新标签 8 的 usage_count 加 1
