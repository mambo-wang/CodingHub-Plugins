...
新增：MCP 工具修改 SHALL 接受可选的 `description`（简短描述，最大 200 字符）和 `tags`（标签名列表）参数。`tags` 参数由 `TagService.resolveOrCreateTags` 自动解析为标签 ID 列表。
...
#### Scenario: 管理员通过 REST API 修改他人工具
...
- **WHEN** 用户调用 `h3_coding_hub_tool_modify`，传入 description="更新的描述"
- **THEN** 工具 description 更新为"更新的描述"

#### Scenario: 修改时传入标签名列表
- **GIVEN** 用户拥有工具 toolId=42，当前无标签关联
- **WHEN** 用户调用 `h3_coding_hub_tool_modify`，传入 tags=["Python", "CLI"]
- **THEN** 系统解析标签名（不存在则创建），关联到工具
